...
<x-admin.card>
    <div class="mb-6">
        <h2 class="text-lg font-semibold">Create New Specialty</h2>
    </div>
    ...
    <form action="{{ route('admin.specialties.store') }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <x-admin.input label="Name" type="text" name="name" id="name" value="{{ old('name') }}" placeholder="Enter specialty name" required />
        </div>

        <div class="flex gap-2">
            {{-- <a href="{{ route('admin.specialties.index') }}">
                <x-admin.button type="secondary">Cancel</x-admin.button>
            </a> --}}
            ...
            <x-admin.button type="primary">Create Specialty</x-admin.button>
        </div>
    </form>
</x-admin.card>
