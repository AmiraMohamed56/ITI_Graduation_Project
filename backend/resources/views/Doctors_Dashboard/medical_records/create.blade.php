...
@section('content')
    <div class="max-w-6xl mx-auto">
        ...
        <form
            ...
            method="POST" enctype="multipart/form-data" class="bg-white rounded-lg shadow">
            @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        @if (!$medicalRecord->exists)
                            ...
                            <div id="patient-preview"
                                class="md:col-span-2 hidden bg-blue-50 border border-blue-200 rounded-lg p-4">
                                <h3 class="font-medium text-blue-900 mb-2">Selected Patient:</h3>
                                <div class="grid grid-cols-2 gap-4 text-sm">
                                    <div>
                                        <span class="text-blue-700">Name:</span>
                                        <span id="preview-name" class="font-medium text-blue-900 ml-2"></span>
                                    </div>
                                    <div>
                                        <span class="text-blue-700">Email:</span>
                                        <span id="preview-email" class="font-medium text-blue-900 ml-2"></span>
                                    </div>
                                </div>
                            </div>
                        @else
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">Patient Email</label>
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <textarea name="symptoms" rows="4" placeholder="Describe the patient's symptoms..."
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('symptoms', $medicalRecord->symptoms) }}</textarea>
                            @error('symptoms')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="diagnosis" rows="4" placeholder="Enter detailed diagnosis..."
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('diagnosis', $medicalRecord->diagnosis) }}</textarea>
                            @error('diagnosis')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="medication" rows="4" placeholder="List prescribed medications with dosage and frequency..."
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('medication', $medicalRecord->medication) }}</textarea>
                            @error('medication')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
